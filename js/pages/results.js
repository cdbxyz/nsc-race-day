...
import { resultInputs, correctionFor } from "./../state.js";
      ...
      el("button.btn.ghost", {
        ...
        onclick: (event) => {
          ...
          savePdf({
            ...
            filename: `nefyn-race-${raceDay.date}-r${race.number}.pdf`,
          }, event.currentTarget);
        },
      }),
      el("button.btn.ghost", {
...
function toCsv() {
  ...
  return [head, ...rows].map((row) => row.map(escape).join(",")).join("\n");
}
